...
mp.add_key_binding("Alt+m", "musixmatch-download", function()
	...
	if response.message.header.status_code == 401 and response.message.header.hint == "renew" then
		error_message(
			"The Musixmatch token has been rate limited. script-opts/lrc.conf explains how to generate a new one."
		)
		return
	end
...
mp.add_key_binding("Alt+n", "netease-download", function()
	...
	for _, song in ipairs(songs) do
		mp.msg.info(
			"Found lyrics for the song with id "
				.. song.id
				.. ", name "
				.. song.name
				.. ", artist "
				...
				.. ", album "
				.. song.album.name
				.. ", url https://music.xianqiao.wang/neteaseapiv2/lyric?id="
				.. song.id
		)
	end
	...
	mp.msg.info(
		"Downloading lyrics for the song with id "
			.. song.id
			.. ", name "
			.. song.name
			.. ", artist "
