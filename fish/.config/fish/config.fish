...
alias ob="obsidian --enable-features=WaylandWindowDecorations --ozone-platform-hint=auto"
alias chr="chromium --enable-features=WaylandWindowDecorations --ozone-platform-hint=auto"
...
alias rm "rm -i"
alias zote "GDK_BACKEND=x11  zotero"
...
function set_temp
	gawk -v temp=$argv -i inplace '{gsub(/temperature\s?+=[0-9]?+\.?[0-9]?+/, "temperature=" temp)}; {print}' ~/.config/hypr/shaders/blue_light.glsl && hyprctl reload config-only
end
